...
function LocationWise({ activeTab }) {
  ...
  function getAverageDataByLocation(data) {
    ...
    data.forEach((entry) => {
      ...
      locationMap[location].monthlyTotal += monthlyAvg;
      locationMap[location].dailyTotal += dailyAvg;
      locationMap[location].yearlyTotal += yearlyAvg;
      locationMap[location].count++;
    });

    const finalAverages = {};
    Object.keys(locationMap).forEach((location) => {
      const { monthlyTotal, dailyTotal, yearlyTotal, count } = locationMap[location];
      finalAverages[location] = {
